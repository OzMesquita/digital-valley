...
public class AtribuirModulos extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			request.setAttribute("pessoas", pessoaDAO.buscarPorNome(nomePessoa, inicio, fim));
			request.setAttribute("perfis", DAOFactory.criarPerfilDAO().Listar());
			request.setAttribute("quantidadeDePaginas", (pessoaDAO.getQuantidadePorNome(nomePessoa)+(fim-inicio-1)) /  (fim - inicio));
