						...
						<div id="corpo">
							<div id="centro">
								<form action="indexJsp.jsp" method="post" name="modulos">
									<div id="user1">
										<div class="form-group">
											<label id="tbl_titu" class="col-md-3" for="selectmultiple">
												<%= mostra %>
											</label> <label class="col-md-3" for="selectmultiple">Módulos
												Disponiveis</label> <label class="col-md-2" for="selectmultiple"
												style="width: 20%;">Módulos Cadastrados</label>
											<div id="tbls" class="col-md-3">
												<input type="hidden" id="selecionado"
													name="usuarioSelecionado" value="" /> <select
													id="selectmultiple" name="selectmultiplePerfil" required
													class="form-control" multiple="multiple" size="15">
													<%if(usuarios != null || perfis!= null){
                                                            if(mostra.equals("Perfil")){ 
                                                                for(int i=0;i<perfis.size();i++){%>
													<option value="<%= perfis.get(i) %>" onclick="mostra()"
														<%if(perfilSelecionado!= null){if(perfis.get(i).getId() == perfilSelecionado.getId()){%>
														selected="true" <%}}%>><%= perfis.get(i).getNome() %> </option>
													<%}}else{
                                                                for(int i=0;i<usuarios.size();i++){%>
													<option value="<%= usuarios.get(i).getId() %>"
														onclick="mostra()"
														<%if(selecionado!= null){if(usuarios.get(i).getId() == selecionado.getId()){%>
														selected="true" <%}}%>><%= usuarios.get(i).getNome() %> </option>
													<%}}
                                                        }else{%>
								...
								<div>
									...
									<div class="col-md-3">
										<select id="selectmultipleDisp"
											name="selectmultipleDisponivel" class="form-control"
								...
								<form action="AdicionarModulos" method="post" name="modulos">
									...
									<div id="btn_salva">
										<input type="hidden" id="lista" name="lista"> <input
											class="btn_pad" id="btn_s" style="margin-top: 1%;"
											type="submit" value="Salvar" title="Salvar Alterações"
											onclick="selecionaTudo()" />
									</div>
								</form>
							</div>
						</div>
